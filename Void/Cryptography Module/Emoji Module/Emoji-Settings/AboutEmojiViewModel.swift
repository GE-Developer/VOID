...
struct AboutEmojiViewModel: AboutEncryptionProtocol {
    ...
    var secondTitle: String? = "Как работает?"
    var secondDescription: String? = "Текст сначала переводится в UTF-8, затем кодируется в Base64, после чего каждый символ заменяется соответствующим эмоджи. Благодаря этому алгоритм поддерживает любые символы и языки."
    
    let thirdTitle: String? = L10n("AboutAES256.fifthTitle")
    var thirdDescription: String? = "Не используйте кодирование в эмоджи для защиты важной информации. Для безопасной передачи данных лучше сочетать его с шифрованием — например, сначала зашифровать текст, а затем преобразовать результат в эмоджи."
